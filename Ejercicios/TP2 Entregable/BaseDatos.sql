...
/* Table: ACTOR                                                 */
/*==============================================================*/
create table ACTOR select * from ACTOR
(
   ALTA                 date                           not null,
   BAJA                 date                           null,
   TELEFONO_PRINCIPAL   varchar(20)                    null,
   MOVIL_PRINCIPAL      varchar(20)                    null,
   EMAIL_PRINCIPAL      varchar(120)                   null,
   OBSERVACIONES        varchar(255)                   null,
   CODIGO               int                            not null,    
   ID                   int IDENTITY(1,1)              not null,
   ID_PAIS              integer                        null,
   constraint PK_ACTOR primary key clustered (ID),
   constraint AK_KEY_2_ACTOR unique (CODIGO)
);
...
(
   TIPO_AFILIADO        varchar(30)                    not null,
   FECHA_ALTA           date                           not null,
   FECHA_BAJA           date                           null,
   OBSERVACIONES        text                           null,
   CODIGO               int                          not null,
   ID                   integer  IDENTITY(1,1)       not null,
   ID_PERSONA_FISICA    integer                        null,
   ID_ESTADO_AFILIADO   integer                        null,
   constraint PK_AFILIADO primary key clustered (ID),
   constraint AK_KEY_2_AFILIADO unique (CODIGO)
);

/*==============================================================*/
/* Table: CUENTAS_AFILIADO                                      */
/*==============================================================*/
...
(
   ...
   ID                   integer  IDENTITY(1,1)         not null,
   ID_AFILIADO          integer                        null,
   constraint PK_CUENTAS_AFILIADO primary key clustered (ID),
   constraint AK_KEY_2_CUENTAS_ unique (ITEM, ID_AFILIADO)
);

/*==============================================================*/
/* Table: DIRECCION_ACTOR                                       */
/*==============================================================*/
...
(
   ...
   LETRA_DOMICILIO      varchar(5)                     not null,
   NOMBRE_CALLE_SIN_NOMENCLAR varchar(120)             not null,
   PISO                 varchar(20)                    not null,
   DEPARTAMENTO         varchar(20)                    not null,
   MANZANA              varchar(20)                    not null,
   ESCALERA             varchar(20)                    not null,
   MONOBLOCK            varchar(20)                    not null,
   ...
   ID_TIPO_DOMICILIO    integer                        null,
   ID_ACTOR             integer                        null,
   constraint PK_DIRECCION_ACTOR primary key clustered (ID),
   constraint AK_KEY_3_DIRECCIO unique (ITEM, ID_ACTOR)
);

/*==============================================================*/
/* Table: ESTADO_AFILIADO                                       */
/*==============================================================*/
...
(
   ...
   ESTADO_AFILIADO      varchar(30)                    not null,
   ID                   integer   IDENTITY(1,1)        not null,
   ID_AFILIADO          integer                        null,
   constraint PK_ESTADO_AFILIADO primary key clustered (ID),
   constraint AK_KEY_2_ESTADO_A unique (SECUENCIA, ID_AFILIADO)
);

/*==============================================================*/
/* Table: ESTADO_CIVIL_PERSONA                                  */
/*==============================================================*/
...
(
   ...
   ID                   integer   IDENTITY(1,1)        not null,
   ID_PERSONA_FISICA    integer                        null,
   constraint PK_ESTADO_CIVIL_PERSONA primary key clustered (ID),
   constraint AK_KEY_2_ESTADO_C unique (ITEM, ID_PERSONA_FISICA)
);
...
(
   ...
   ITEM                 smallint                       not null,
   ID                   integer    IDENTITY(1,1)       not null,
   ID_PERSONA_FISICA_PERSONA integer                   null,
   ID_PERSONA_FISICA_PARIENTE integer                  null,
   constraint PK_FAMILIARES primary key clustered (ID),
   constraint AK_KEY_2_FAMILIAR unique (ITEM, ID_PERSONA_FISICA_PERSONA)
);

/*==============================================================*/
/* Table: FAMILIAR_AFILIADO                                     */
/*==============================================================*/
...
(
   ...
   ITEM                 smallint                       not null,
   ID                   integer    IDENTITY(1,1)       not null,
   ID_FAMILIARES        integer                        null,
   ID_AFILIADO          integer                        null,
   constraint PK_FAMILIAR_AFILIADO primary key clustered (ID),
   constraint AK_KEY_2_FAMILIARi unique (ITEM, ID_AFILIADO)
);
...
(
   ...
   ESCOLARIDAD_ACTUAL   varchar(30)                    not null,
   TIENE_DISCAPACIDAD   smallint                       not null,
   NRO_DOCUMENTO        int                            not null,
   TIPO_DOCUMENTO       varchar(30)                    not null,
   ID                   integer    IDENTITY(1,1)       not null,
   ID_ACTOR             integer                        null,
   ID_ESTADO_CIVIL_PERSONA integer                        null,
   constraint PK_PERSONA_FISICA primary key clustered (ID),
   constraint AK_KEY_2_PERSONA_ unique (NRO_DOCUMENTO, TIPO_DOCUMENTO)
);
...
(
   ...
   ID                   integer   IDENTITY(1,1)        not null,
   constraint PK_PROVIENCIA primary key clustered (ID),
   constraint AK_KEY_2_PROVIENC unique (CODIGO)
);
...
CREATE TABLE PAIS (
  ID int IDENTITY(1,1),
  PAIS_CODIGO char(3) NOT NULL DEFAULT '',
  PAIS_NOMBRE char(52) NOT NULL DEFAULT '',
  PAIS_CONTINENTE varchar(50) NOT NULL DEFAULT 'Asia',
  PAIS_REGION varchar(26) NOT NULL DEFAULT '',
  PAIS_AREA float NOT NULL DEFAULT '0.00',
  PAIS_INDEPENDENCIA smallint DEFAULT NULL,
  PAIS_POBLACION int NOT NULL DEFAULT '0',
  PAIS_EXOECTATIVA_DE_VIDA float DEFAULT NULL,
  PAIS_PRODUCTO_INTERNO_BRUTO float DEFAULT NULL,
  PAIS_PRODUCTO_INTERNO_BRUTO_ANTIGUO float DEFAULT NULL,
  PAIS_NOMBRE_LOCAL varchar(45) NOT NULL DEFAULT '',
  PAIS_GOBIERNO varchar(45) NOT NULL DEFAULT '',
  PAIS_JEFE_DE_ESTADO varchar(60) DEFAULT NULL,
  PAIS_CAPITAL int DEFAULT NULL,
  PAIS_CODIGO2 char(2) NOT NULL DEFAULT '',
   constraint PK_PAIS primary key clustered (ID),
   constraint AK_KEY_1_PAIS unique (PAIS_CODIGO)
);
...
INSERT INTO Pais (PAIS_CODIGO, PAIS_NOMBRE, PAIS_CONTINENTE, PAIS_REGION, PAIS_AREA, PAIS_INDEPENDENCIA, PAIS_POBLACION, PAIS_EXOECTATIVA_DE_VIDA, PAIS_PRODUCTO_INTERNO_BRUTO, PAIS_PRODUCTO_INTERNO_BRUTO_ANTIGUO, PAIS_NOMBRE_LOCAL, PAIS_GOBIERNO, PAIS_JEFE_DE_ESTADO, PAIS_CAPITAL, PAIS_CODIGO2) VALUES
...
alter table AFILIADO
   add constraint FK_PF foreign key (ID_PERSONA_FISICA)
      references PERSONA_FISICA (ID);

alter table AFILIADO
   add constraint FK_ESTADO_AFILIADO foreign key (ID_ESTADO_AFILIADO)
      references ESTADO_AFILIADO (ID);

alter table CUENTAS_AFILIADO
   add constraint FK_AFILIADO foreign key (ID_AFILIADO)
...
alter table DIRECCION_ACTOR
   add constraint FK_DOMICILIO foreign key (ID_TIPO_DOMICILIO)
      references TIPO_DOMICILIO (ID);

alter table DIRECCION_ACTOR
   add constraint FK_actor foreign key (ID_ACTOR)
      references ACTOR (ID);
...
alter table FAMILIAR_AFILIADO
   add constraint FK_FAMILIARES foreign key (ID_FAMILIARES)
...
alter table PERSONA_FISICA
   add constraint FK_ECP foreign key (ID_ESTADO_CIVIL_PERSONA)
